  ...
  return (
    <Card className="h-full border-stone-200 shadow-none">
      ...
      <CardContent className="space-y-3 text-sm text-muted-foreground">
        ...
        {voucher.usageLimit > 0 ? <p>{t("voucher_usage_limit")}: {voucher.usageLimit}</p> : null}
        {voucher.usageCount > 0 ? <p>{t("voucher_usage_count")}: {voucher.usageCount}</p> : null}
        {voucher.orderId ? <p>{t("voucher_used_on_order")}: {voucher.orderId}</p> : null}
        <p>
