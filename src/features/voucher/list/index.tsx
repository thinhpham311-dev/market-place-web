...
import { TicketPercent, WalletCards, CircleCheckBig, Clock3, Store } from "lucide-react";
...
import { useFetchData as useFetchShopData } from "@/features/shop/hooks";
import VoucherListRoot from "./voucher-list-root";

function VoucherListContent() {
  ...
  const { shopHref, shopInfo } = useFetchShopData({
    shop_id: resolvedShopId || "",
    enabled: Boolean(resolvedShopId),
  });
        ...
        <Card className="border-none bg-gradient-to-r from-orange-50 via-white to-amber-50 shadow-sm md:col-span-3">
          <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            {resolvedShopId ? (
              ...
              >
                <Link href={shopHref || `/shop/shop-s.${resolvedShopId}`}>
                  <Store className="h-4 w-4 text-orange-600" />
                  <span>
                    {t("voucher_back_to_shop")}
                            ...
                            <div className="mt-2 flex flex-wrap items-center justify-between gap-3 text-sm text-muted-foreground md:justify-end md:gap-4">
                              ...
                              <div className="flex items-center gap-2">
                                <Button asChild type="button" size="sm" variant="outline">
                                  <Link
                                    href={`/vouchers/${voucher.discountId}${voucher.shopId || resolvedShopId ? `?shopId=${voucher.shopId || resolvedShopId}` : ""}`}
                                  >
...
export default function VoucherListPage({
  ...
  return (
    <VoucherListRoot shopId={shopId} limit={limit} page={page}>
      <VoucherListContent />
    </VoucherListRoot>
  );
}
