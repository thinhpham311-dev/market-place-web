 ...
 */
const makeShoppingCartBase = (dynamicReducerKey: string, storeKey: string) => (state: RootState) =>
 ...
 */
const selectorCache: Record<string, Record<string, ReturnType<typeof makeShoppingCartBase>>> = {};

/**
 * Public API — ALWAYS use reducerKey, not reducerKey
 */
export const selectShoppingCart = (dynamicReducerKey: string, storeKey: string) => {
  if (!selectorCache[dynamicReducerKey]) {
    selectorCache[dynamicReducerKey] = {};
  }

  if (!selectorCache[dynamicReducerKey][storeKey]) {
    selectorCache[dynamicReducerKey][storeKey] = makeShoppingCartBase(dynamicReducerKey, storeKey);
  }
