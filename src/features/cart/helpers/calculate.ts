...
const recalculateTotals = (cart: ICartModel) => {
  ...
  cart.cart_selected_items_count = cart.cart_selected_items?.length || 0;
  ...
  if (cart.cart_items.length === 0) {
    ...
    const hasSelected = Boolean(cart.cart_selected_items && cart.cart_selected_items.length > 0);
    const baseSubTotal = hasSelected ? cart.cart_selected_items_total : 0;

    const discount = cart.cart_total_discount || 0;
    const netSubTotal = Math.max(0, baseSubTotal - discount);

    cart.cart_estimated_shipping = calculateEstimatedShipping(baseSubTotal);
    cart.cart_estimated_tax = calculateEstimatedTax(netSubTotal);
