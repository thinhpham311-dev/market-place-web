...
export const makeSelectInfiniteScrollState = (storeKey: string) =>
  createSelector(
    (state: RootState) =>
      state[`${INFINITE_SCROLL}_${storeKey}`]
        ? state[`${INFINITE_SCROLL}_${storeKey}`].state
        : null,
    (infiniteScrollState) => ({
...
const infiniteScrollSelectorsCache: Record<
  string,
  ReturnType<typeof makeSelectInfiniteScrollState>
> = {};
